...
def load_ppo(filename, env=None):
    '''
    Loads a previously saved DQN model
    '''
    model = PPO2.load(filename, env=env)
    return model
...
def stable_ppo(X_train, y_train, timesteps, save=False, log_path=None, log_prefix='ppo', filename=None):
    ...
    model.learn(total_timesteps=timesteps, log_interval=100000, call_back=checkpoint_callback)
...
def finetune_pretrained_model(timesteps, model, save=False, log_path=None, log_prefix=None, filename=None, per=False):
    '''
    Finetunes a pre-trained DQN model
    '''        
...
def train_dqn_model(steps, model_path, X_train, y_train, seed, parent_dir=None, log_prefix=None, filename='finetuned_model'):
    dir_name = f'seed_{seed}_{steps}'
    path = os.path.join(parent_dir, dir_name)
    os.mkdir(path)
    training_env = create_env(X_train, y_train)
    pretrained_model = load_dqn(model_path, env=training_env)
    model = finetune_pretrained_model(timesteps=steps, model=pretrained_model, save=True, log_path=path, log_prefix=log_prefix,
                                      filename=filename)
    training_env.close()
    return model
...
def remove_no_anems(df, frac):
    filtered_df = df[df["label"] == 0]
    sample_size = int(len(filtered_df) * (frac))
    sampled_df = filtered_df.sample(sample_size, random_state=42)
    df_dropped = df.drop(sampled_df.index)
    return df_dropped
